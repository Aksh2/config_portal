...
import com.rummycircle.pageobjects.newwebadmin.branding.ManageBrandingPage;
...
public class BrandingTest extends NewBaseTest {
	...
	public void createBrandingTest() {
		...
		CreateBrandingPage brandingPage = new CreateBrandingPage(DriverManager
				.getWebDriver().get());
		...
		Assert.assertEquals(brandingPage.getTextOfBrandingPreview(),
				brandingDetails, "Branding Details", false);
		brandingPage.clickCloseButton();
		brandingPage.fillMobileTournamentDetails(custom
				.getProperty("image.src"));
		...
		// Search for the created branding and check the details in the preview
		// link
		ManageBrandingPage manageBrandingPage = new ManageBrandingPage(
				DriverManager.getWebDriver().get());
		manageBrandingPage.fillSearchTerm(name);
		manageBrandingPage.clickManageBrandingPreviewLink();
		Assert.assertEquals(manageBrandingPage.getTextOfBrandingPreview(),
				brandingDetails, "Manage Branding Details", false);
		manageBrandingPage.clickCloseButton();

		manageBrandingPage.clickManageBrandingEditLink();
		Assert.assertEquals(manageBrandingPage.getValueOfBrandingNameInput(),
				name, "Branding Name", false);
		Assert.assertEquals(
				manageBrandingPage.getValueOfBrandingDetailsInput(),
				brandingDetails, "Branding Details", false);
		Assert.assertEquals(
				manageBrandingPage.getValueOfMobileBrandingDetailsInput(),
				custom.getProperty("image.src"), "Mobile Branding Details",
				false);

	}

	@Test(description = "Negative tests of creating a branding", dataProvider = "negativeValues")
	public void createBrandingNegativeTests(String name,
		...
		brandingPage.clickCreateButton();
		Assert.assertNotNull(brandingPage.getErrorMessageOfBranding(),
				"Error Message", true);
		brandingPage.clickCloseButtonInCreateBrandingDialogue();
	...
	public Object[][] getValues() {
		return new Object[][] { { "", getRandomString(5) },
				{ getRandomString(5), "" } };
	}
